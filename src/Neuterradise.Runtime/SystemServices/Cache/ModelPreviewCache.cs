...
public sealed class ModelPreviewCache
{
    ...
    private readonly ConcurrentDictionary<ModelPreviewCacheKey, byte> _activeGenerations = new();

    public ModelPreviewCache(CachePaths paths, CacheFilePublisher publisher, PersistentCacheBudget budget)
    {
        ...
        string extension = ".png")
    {
        ...
        var physicalPath = _paths.ResolveContainedPath(CacheFamily.ModelPreviews, relativePath);
    ...
    public bool TryBeginGeneration(ModelPreviewCacheKey key, out IDisposable? generationScope)
    {
        ArgumentNullException.ThrowIfNull(key);
    ...
    public bool Invalidate(ModelPreviewCacheKey key, string extension = ".png")
    {
        ArgumentNullException.ThrowIfNull(key);
        var relativePath = key.GetRelativePath(extension);
        var physicalPath = _paths.ResolveContainedPath(CacheFamily.ModelPreviews, relativePath);
    ...
    {
        var assetDir = Path.Combine(_paths.ModelPreviewsPath, assetId.ToString("N"));
    ...
    {
        if (!Directory.Exists(_paths.ModelPreviewsPath))
        {
            return 0;
        }
        ...
        foreach (var file in Directory.EnumerateFiles(_paths.ModelPreviewsPath, pattern, SearchOption.AllDirectories))
        {
            try
            {
                File.Delete(file);
