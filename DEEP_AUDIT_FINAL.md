...
| X26 | `src/Neuterradise.Runtime/SystemServices/Jobs/JobScheduler.cs` — scheduler shutdown cancellation; `src/Neuterradise.Runtime/SystemServices/Jobs/Handlers/StorageJobResultMapper.cs` and `HashAssetJobHandler.cs` — cancellation-to-job-result mapping on resumable work. |
...
| X59 | EVIDENCE boundary: `scripts/package-win-x64.ps1`; `src/Neuterradise.Profiling.Worker/ProfilingWorkerHost.cs`, `Dispatching/ProfilingRequestDispatcher.cs`, and `FaceAnalysis/FaceAnalyzer.cs`; `src/Neuterradise.Profiling.Protocol/ProfilingContracts.cs` and `ProfilingProtocol.cs`; plus the packaged YuNet/SFace artifacts. |
